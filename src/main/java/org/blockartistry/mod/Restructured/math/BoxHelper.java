...
public final class BoxHelper {
	...
	public static class RegionStats {
		...
		public String toString() {
			return String.format("[area: %d; mean: %d; variance: %d; deviation: %d]", area, mean, variance, deviation);
		}
	};
	
    public static RegionStats getRegionStats(World world, StructureBoundingBox worldBB, StructureBoundingBox structBB)
    {
        ...
        for (int z = structBB.minZ; z <= structBB.maxZ; ++z)
            for (int x = structBB.minX; x <= structBB.maxX; ++x)
                if (worldBB.isVecInside(x, 64, z))
                    data.add(Math.max(world.getTopSolidOrLiquidBlock(x, z), avgGroundLevel));
